...
#[test]
fn generic_branch_list_show_log_and_local_forget_compose() {
    ...
    let mut pile = Pile::open(&source).unwrap();
    pile.refresh().unwrap();
    ...
    for signing_key in [&first_key, &second_key] {
        pile.append_pin_assertion(sign_branch_assertion(
            signing_key,
            name,
            commit,
            BranchRank::ROOT,
    ...
    Command::cargo_bin("trible")
        ...
        .stdout(predicates::str::contains(
            "Physical asserted-pin arrival order",
        ))
        .stdout(predicates::str::contains(format!(
        ...
        for record in &mut records {
            ...
            let forgotten = matches!(
                ...
                PileRecordContent::PinAssertion { assertion }
                    if assertion.identity() == &first_identity.pin_identity()
            );
            if !forgotten {
    ...
    assert!(snapshot.for_pin(&first_identity.pin_identity()).is_empty());
    assert_eq!(snapshot.for_pin(&second_identity.pin_identity()).len(), 1);
    assert_eq!(snapshot.for_pin(&source_identity.pin_identity()).len(), 1);
