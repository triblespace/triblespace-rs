...
//!
//! A warm resident sample still includes the status upload, the seventeen
...
fn main() {
    ...
    println!(
        "# setup=excluded; warm_resident=212 control H2D bytes (8 status + 17*12 dispatch records) + all allocations/kernels + exactly one final packed D2H"
    );
    println!(
