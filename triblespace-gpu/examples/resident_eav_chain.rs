...
//! eighteen host-exact local launches use direct static rectangles rather than
//! uploaded indirect-dispatch records. This is therefore an end-to-end warm
...
fn main() {
    ...
    println!(
        "# setup=excluded; warm_resident=8 control H2D bytes (status only; 18 host-exact launches use direct rectangles) + all allocations/kernels + exactly one final packed D2H"
    );
    println!(
