...
# Reproducible old/current demand-curve experiment.
...
#   OLD_FRONTIER_STATS (0 or 1), RAYON_NUM_THREADS, DEMAND_CURVE_CACHE_ROOT
...
OLD_FRONTIER_STATS=${OLD_FRONTIER_STATS:-0}
[[ "$OLD_FRONTIER_STATS" == 0 || "$OLD_FRONTIER_STATS" == 1 ]] || {
    echo "OLD_FRONTIER_STATS must be 0 or 1" >&2
    exit 2
}
...
prepare_lock() {
    local label=$1
    local worktree=$2
    local source_lock="$worktree/Cargo.lock"
    if [[ ! -f "$source_lock" ]]; then
        echo "resolving $label dependency lock outside the timed region" >&2
        cargo generate-lockfile --manifest-path "$worktree/Cargo.toml" \
            >"$OUT/build-logs/dependency-lock-$label.log" 2>&1
    else
        printf 'reusing %s\n' "$source_lock" \
            >"$OUT/build-logs/dependency-lock-$label.log"
    fi
    [[ -f "$source_lock" ]] || {
        echo "dependency resolution did not create $source_lock" >&2
        exit 2
    }
    cp "$source_lock" "$OUT/locks/Cargo.lock-$label"
    shasum -a 256 "$OUT/locks/Cargo.lock-$label" | awk '{print $1}'
}

OLD_DEPENDENCY_LOCK_SHA256=$(prepare_lock old "$OLD_WORKTREE")
CURRENT_DEPENDENCY_LOCK_SHA256=$(prepare_lock current "$CURRENT_WORKTREE")
...
build_subject() {
    ...
    local dependency_lock_sha256=$5
    shift 5
    [[ "$(shasum -a 256 "$worktree/Cargo.lock" | awk '{print $1}')" == \
        "$dependency_lock_sha256" ]] || {
        echo "$label worktree does not contain its frozen dependency lock" >&2
        exit 2
    }
...
if [[ "$OLD_FRONTIER_STATS" == 1 ]]; then
    build_subject old "$OLD_WORKTREE" "$OLD_REV" old \
        "$OLD_DEPENDENCY_LOCK_SHA256" --cfg demand_frontier_stats
else
    build_subject old "$OLD_WORKTREE" "$OLD_REV" old \
        "$OLD_DEPENDENCY_LOCK_SHA256"
fi
build_subject current "$CURRENT_WORKTREE" "$CURRENT_REV" current \
    "$CURRENT_DEPENDENCY_LOCK_SHA256" \
...
run_one() {
    ...
    case "$label" in
        old) dependency_lock_sha256=$OLD_DEPENDENCY_LOCK_SHA256 ;;
        current|current-w1) dependency_lock_sha256=$CURRENT_DEPENDENCY_LOCK_SHA256 ;;
        *) echo "unknown subject label: $label" >&2; exit 2 ;;
    esac
...
awk -F '\t' \
    ...
    -v old_lock="$OLD_DEPENDENCY_LOCK_SHA256" \
    -v current_lock="$CURRENT_DEPENDENCY_LOCK_SHA256" '
    ...
    }
    {
        ...
        if (engine == old_revision) {
            ...
            if (value("dependency_lock") != old_lock)
                fail(sprintf("old dependency lock mismatch at line %d", NR))
            ...
            if (value("dependency_lock") != current_lock)
                fail(sprintf("current dependency lock mismatch at line %d", NR))
        } else {
