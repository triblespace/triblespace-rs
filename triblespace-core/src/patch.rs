...
impl<const KEY_LEN: usize, O: KeySchema<KEY_LEN>, V> Head<KEY_LEN, O, V> {
    ...
    pub(crate) fn union(mut this: Self, mut other: Self, at_depth: usize) -> Self {
        match this.equality_mode(&other) {
            EqualityMode::ExactKey => {
                return Self::union_exact_key_pair(this, other, at_depth);
            }
            EqualityMode::FingerprintImpossible => {}
            EqualityMode::Fingerprint if this.hash() == other.hash() => return this,
            EqualityMode::Fingerprint => {}
        }
        ...
        V: Send + Sync,
    {
        match self.equality_mode(other) {
            ...
            EqualityMode::FingerprintImpossible => {}
            EqualityMode::Fingerprint if self.hash() == other.hash() => return None,
            EqualityMode::Fingerprint => {}
        }
        if self.first_divergence(other, at_depth).is_some() {
            return Some(self.clone());
