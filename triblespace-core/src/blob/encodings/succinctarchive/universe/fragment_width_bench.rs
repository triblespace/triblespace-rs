//! Reproducible probe for choosing one fixed compressed-universe fragment width.
//!
//! Run with:
//! `cargo test -p triblespace-core --release fragment_width_matrix -- --ignored --nocapture`
//!
//! On the 2026-08-08 M4 Max release build, 16-byte fragments were the coherent
//! compressed choice. Against four-byte fragments they cut frozen arena bytes
//! by 35.1% on the 101,499-row block-DAG corpus and 15.6% on the 100,000-row
//! mixed corpus; whole archive construction improved by about 1.8-2.2x and
//! the bound-EA proposal cohort by about 1.8x. Eight-byte fragments were 1,818
//! bytes (0.07%) smaller only on the mixed corpus, while losing materially on
//! every timed operation. `OrderedUniverse` remains the latency oracle but is
//! 25% larger than 16-byte fragments on both corpora.
//!
//! The reported universe byte count is the entire frozen [`ByteArea`]:
//! dictionary, DAC payload/flag sections, level tables, and alignment. It does
//! not include heap-rebuilt Rank9 indexes over DAC continuation flags. Raw
//! Succinct and detached top-level Rank9 artifact sizes are reported
//! separately and asserted byte-identical across runtime universe widths.
...
use super::{FragmentedUniverse, OrderedUniverse, Universe};
...
fn build_universe<U>(values: &[RawInline]) -> (U, Bytes, Duration)
where
    ...
    U::Meta: Copy,
{
    ...
    for _ in 0..3 {
        let (universe, bytes, duration) = build_universe::<U>(&corpus.domain);
        builds.push(duration);
...
fn corpus_matrix(corpus: &Corpus) {
    ...
    universe_row::<FragmentedUniverse<4>>(corpus, "fragment-4");
    universe_row::<FragmentedUniverse<8>>(corpus, "fragment-8");
    ...
    let (raw_4, rank9_4) = archive_row::<FragmentedUniverse<4>>(corpus, "fragment-4");
    let (raw_8, rank9_8) = archive_row::<FragmentedUniverse<8>>(corpus, "fragment-8");
    let (raw_16, rank9_16) = archive_row::<FragmentedUniverse<16>>(corpus, "fragment-16");

    assert_eq!(raw_ordered.bytes, raw_4.bytes);
    assert_eq!(raw_4.bytes, raw_8.bytes);
    assert_eq!(raw_8.bytes, raw_16.bytes);
    assert_eq!(rank9_ordered.bytes, rank9_4.bytes);
    assert_eq!(rank9_4.bytes, rank9_8.bytes);
    assert_eq!(rank9_8.bytes, rank9_16.bytes);

    let cross_attached = SuccinctArchive::<FragmentedUniverse<16>>::from_blob_pair(raw_4, rank9_4)
        .expect("Rank9 accelerator must attach across runtime universe widths");
    assert_eq!(cross_attached.iter().count(), corpus.set.len());
}

#[test]
#[ignore = "manual release-mode representation matrix"]
fn fragment_width_matrix() {
    println!("kind,corpus,implementation,bytes,build_ms,access_ms,search_ms,lower_ms");
