...
/// V3 signed branch assertion — one complete fixed 256-byte record.
#[derive(TryFromBytes, IntoBytes, Immutable, KnownLayout, Copy, Clone)]
#[repr(C)]
struct BranchAssertionHeaderV3 {
    magic_marker: RawId,
    assertion: [u8; BRANCH_ASSERTION_LEN],
    reserved: [u8; BRANCH_ASSERTION_RESERVED_LEN],
}

impl BranchAssertionHeaderV3 {
    fn new(assertion: BranchAssertion) -> Self {
        Self {
            magic_marker: MAGIC_MARKER_BRANCH_ASSERTION_V3,
            assertion: assertion.encode(),
            reserved: [0u8; BRANCH_ASSERTION_RESERVED_LEN],
        }
    }
}
...
pub enum PileRecordContent {
    ...
    /// A mutable local-pin update encoded with a historical branch marker.
    Pin {
        /// The pin being updated.
        pin_id: Id,
        /// The pin's new value.
        head: Inline<Handle<SimpleArchive>>,
    },
    /// A mutable local-pin tombstone encoded with a historical branch marker.
    PinTombstone {
        /// The pin being tombstoned.
        pin_id: Id,
    },
    /// A strictly verified, grow-only branch assertion.
    BranchAssertion {
...
fn decode_record(bytes: &[u8], offset: usize) -> Result<DecodedPileRecord, ReadError> {
    ...
    match magic {
        ...
        MAGIC_MARKER_BRANCH_ASSERTION_V3 => {
            let (header, _) =
                BranchAssertionHeaderV3::try_read_from_prefix(bytes).map_err(|_| corrupt())?;
...
/// Error returned by the grow-only branch-assertion store.
#[derive(Debug)]
pub enum PileAssertionError {
...
impl std::fmt::Display for PileAssertionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::ReadError(err) => write!(f, "pile replay failed: {err}"),
            Self::IoError(err) => write!(f, "assertion append failed: {err}"),
            Self::KeyCollision(err) => std::fmt::Display::fmt(err, f),
        }
    }
}

impl std::error::Error for PileAssertionError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
...
impl From<ReadError> for PileAssertionError {
    fn from(err: ReadError) -> Self {
        Self::ReadError(err)
    }
}

impl From<std::io::Error> for PileAssertionError {
    fn from(err: std::io::Error) -> Self {
        Self::IoError(err)
    }
}

impl From<AssertionKeyCollision> for PileAssertionError {
    fn from(err: AssertionKeyCollision) -> Self {
...
// Generic durability hook: appended records (blobs, mutable-pin updates,
...
impl PinStore for Pile {
    ...
    type ListIter<'a> = PilePinStoreIter;

    fn pins<'a>(&'a mut self) -> Result<Self::ListIter<'a>, Self::PinsError> {
        // Ensure newly appended records are applied before enumerating
        // pins so external writers are visible to callers.
        self.refresh()?;
    ...
    /// allowing piles to reference external data and serve as pin-only stores.
    ///
    ...
    fn update(
        ...
        let res = (|| {
            ...
            // V3 legacy pin/tombstone records: fixed 256-byte header, no data, so the
            // record is exactly one 256-byte unit — keeping a pure-V3 pile
            // 256-aligned throughout (pins write under the exclusive lock).
            self.dirty = true;
            ...
            if written != expected {
                return Err(PileWriteError::IoError(std::io::Error::new(
                    std::io::ErrorKind::WriteZero,
                    "failed to write pin header",
                )));
            }
            match self.apply_next().map_err(PileWriteError::from)? {
                Some(Applied::Pin { id: pin_id, hash }) if matches!(new, Some(new) if pin_id == id && hash == new.into()) => {
                    Ok(PushResult::Success())
                }
                Some(Applied::PinTombstone { id: pin_id }) if new.is_none() && pin_id == id => {
                    Ok(PushResult::Success())
                }
                Some(_) => Err(PileWriteError::IoError(std::io::Error::other(
                    "unexpected record after pin write",
                ))),
                None => Err(PileWriteError::IoError(std::io::Error::other(
                    "pin missing after write",
                ))),
            }
        })();
        let unlock_res = self.file.unlock();
...
impl BranchAssertionStore for Pile {
    ...
    /// Append one verified assertion as a fixed 256-byte Pile record.
    ///
    /// Unlike the legacy pin and weak-pin APIs, this method fulfils the trait's
    /// durability promise before returning: both fresh appends and idempotent
    /// duplicates pass through `sync_all` while the exclusive Pile lock is
    /// held. A fresh record is decoded and applied through the normal replay
    /// path before the durability barrier, so the disk parser remains the one
    ...
    fn append_assertion(&mut self, assertion: BranchAssertion) -> Result<(), Self::Error> {
        ...
        let result = (|| {
            ...
            let header = BranchAssertionHeaderV3::new(assertion);
            self.dirty = true;
            self.file.write_all(header.as_bytes())?;
            match self.apply_next()? {
                Some(Applied::BranchAssertion { id }) if id == assertion.id() => {}
                Some(_) => {
                    return Err(PileAssertionError::IoError(std::io::Error::other(
                        "unexpected record after branch assertion write",
                    )));
                }
                None => {
                    return Err(PileAssertionError::IoError(std::io::Error::other(
                        "branch assertion missing after write",
                    )));
                }
            }
            self.file.sync_all()?;
...
impl Pile {
    ...
    /// lock. Like mutable-pin updates, the record is **not durable** until
...
impl WeakPinStore for Pile {
    ...
    /// mutable-pin updates: call [`Pile::flush`] to make it crash-durable.
...
mod tests {
    ...
    fn branch_assertion(key: u8, name: u8, commit: u8) -> BranchAssertion {
        BranchAssertion::sign(
    ...
    #[test]
    fn branch_assertion_record_is_fixed_verified_idempotent_and_durable() {
        let dir = tempfile::tempdir().unwrap();
        let path = fresh_empty_pile_path(&dir, "assertion.pile");
        let assertion = branch_assertion(7, 11, 19);

        let mut pile = Pile::open(&path).unwrap();
        pile.append_assertion(assertion).unwrap();
        assert!(!pile.dirty, "durable assertion append must cross sync_all");
        assert_eq!(
        ...
        assert_eq!(&bytes[..16], &MAGIC_MARKER_BRANCH_ASSERTION_V3);
        assert_eq!(&bytes[16..16 + BRANCH_ASSERTION_LEN], &assertion.encode());
        assert!(bytes[16 + BRANCH_ASSERTION_LEN..]
            .iter()
            .all(|byte| *byte == 0));
        ...
        let mut second_handle = Pile::open(&path).unwrap();
        second_handle.append_assertion(assertion).unwrap();
        assert_eq!(std::fs::metadata(&path).unwrap().len(), len);
        second_handle.close().unwrap();

        let records: Vec<_> = PileRecords::open(&path)
            .unwrap()
            .map(Result::unwrap)
            .collect();
        ...
        assert!(matches!(
            records[0].content,
            PileRecordContent::BranchAssertion { assertion: decoded }
                if decoded == assertion
        ));

        let mut reopened = Pile::open(&path).unwrap();
        let snapshot = reopened.assertion_snapshot().unwrap();
        assert_eq!(
            snapshot.iter().copied().collect::<Vec<_>>(),
            vec![assertion]
        );
    ...
    #[test]
    fn truncated_branch_assertion_reports_and_amputates_at_the_record_boundary() {
        let dir = tempfile::tempdir().unwrap();
        let first = BranchAssertionHeaderV3::new(branch_assertion(7, 11, 19));
        let second = BranchAssertionHeaderV3::new(branch_assertion(7, 11, 23));

        for prefix_len in [1usize, 16, 175, V3_HEADER_LEN - 1] {
            let path = fresh_empty_pile_path(&dir, &format!("truncated-{prefix_len}.pile"));
            let mut bytes = first.as_bytes().to_vec();
            bytes.extend_from_slice(&second.as_bytes()[..prefix_len]);
            ...
            assert_eq!(pile.assertion_snapshot().unwrap().len(), 1);
            pile.close().unwrap();
        }
    }

    #[test]
    fn cat_merge_unions_branch_assertions_without_log_order_semantics() {
        let dir = tempfile::tempdir().unwrap();
        let left_path = fresh_empty_pile_path(&dir, "left.pile");
        let right_path = fresh_empty_pile_path(&dir, "right.pile");
        let first = branch_assertion(7, 11, 19);
        let second = branch_assertion(7, 11, 23);

        let mut left = Pile::open(&left_path).unwrap();
        left.append_assertion(first).unwrap();
        left.close().unwrap();

        let mut right = Pile::open(&right_path).unwrap();
        right.append_assertion(second).unwrap();
        right.append_assertion(first).unwrap();
        right.close().unwrap();

        let right_bytes = std::fs::read(&right_path).unwrap();
        let mut left_file = OpenOptions::new().append(true).open(&left_path).unwrap();
        left_file.write_all(&right_bytes).unwrap();
        left_file.sync_all().unwrap();
        drop(left_file);

        let mut merged = Pile::open(&left_path).unwrap();
        let snapshot = merged.assertion_snapshot().unwrap();
        assert_eq!(snapshot.len(), 2);
        let assertions: Vec<_> = snapshot.iter().copied().collect();
        assert!(assertions.contains(&first));
        assert!(assertions.contains(&second));
        merged.close().unwrap();
    }
    ...
    fn close_flushes_only_mutations_by_this_handle() {
        ...
        let pin = Id::new([9u8; 16]).unwrap();
        writer.update(pin, None, Some(handle.transmute())).unwrap();
        assert!(writer.dirty);
        writer.flush().unwrap();
        assert!(!writer.dirty);
        ...
        assert!(!writer.dirty);
        writer
            .update(pin, Some(handle.transmute()), Some(handle.transmute()))
            .unwrap();
        assert!(!writer.dirty);
    ...
    #[test]
    fn blob_after_pin_is_clean() {
        let dir = tempfile::tempdir().unwrap();
        let path = fresh_empty_pile_path(&dir, "pile.pile");

        let mut pile: Pile = Pile::open(&path).unwrap();

        let pin_id = Id::new([1; 16]).unwrap();
        let head = Inline::<Handle<SimpleArchive>>::new([2; 32]);
        pile.update(pin_id, None, Some(head)).unwrap();
    ...
    #[test]
    fn insert_after_pin_preserves_head() {
        let dir = tempfile::tempdir().unwrap();
        let path = fresh_empty_pile_path(&dir, "pile.pile");
        ...
        let pin_id = Id::new([1u8; 16]).unwrap();
        pile.update(pin_id, None, Some(handle1.transmute()))
            .unwrap();
        ...
        let mut pile: Pile = Pile::open(&path).unwrap();
        pile.amputate().unwrap();
        let head = pile.head(pin_id).unwrap();
        assert_eq!(head, Some(handle1.transmute()));
        pile.close().unwrap();
    }

    #[test]
    fn pin_update_survives_manual_flush() {
        let dir = tempfile::tempdir().unwrap();
        let path = fresh_empty_pile_path(&dir, "pile.pile");
        ...
        let handle = {
            ...
            let handle = pile.put::<UnknownBlob, _>(blob).unwrap();
            pile.update(pin_id, None, Some(handle.transmute())).unwrap();
            pile.flush().unwrap();
            std::mem::forget(pile);
            handle
        };
        ...
        let mut pile: Pile = Pile::open(&path).unwrap();
        pile.amputate().unwrap();
        assert_eq!(pile.head(pin_id).unwrap(), Some(handle.transmute()));
        assert!(std::fs::metadata(&path).unwrap().len() > 0);
        pile.close().unwrap();
    }

    #[test]
    fn pin_tombstone_removes_head_and_listing() {
        let dir = tempfile::tempdir().unwrap();
        let path = fresh_empty_pile_path(&dir, "pile.pile");
        ...
        let h = pile.put::<UnknownBlob, _>(blob).unwrap();
        let pin_id = Id::new([7u8; 16]).unwrap();
        pile.update(pin_id, None, Some(h.transmute())).unwrap();
        pile.flush().unwrap();

        assert_eq!(pile.head(pin_id).unwrap(), Some(h.transmute()));

        pile.update(pin_id, Some(h.transmute()), None).unwrap();
        pile.flush().unwrap();

        assert_eq!(pile.head(pin_id).unwrap(), None);
    ...
    #[test]
    fn pin_update_detects_conflict() {
        let dir = tempfile::tempdir().unwrap();
        let path = fresh_empty_pile_path(&dir, "pile.pile");
        ...
        let pin_id = Id::new([2u8; 16]).unwrap();
        pile.update(pin_id, None, Some(handle1.transmute()))
            .unwrap();
        ...
        match pile
            .update(pin_id, Some(handle2.transmute()), Some(handle2.transmute()))
            .unwrap()
        {
        ...
        assert_eq!(pile.head(pin_id).unwrap(), Some(handle1.transmute()));
        pile.close().unwrap();
    }

    #[test]
    fn pin_update_conflict_returns_current_head() {
        let dir = tempfile::tempdir().unwrap();
        let path = fresh_empty_pile_path(&dir, "pile.pile");
    ...
    fn update_conflict_returns_current_head() {
        ...
        let h1 = pile.put::<UnknownBlob, _>(blob1).unwrap();
        let pin_id = Id::new([1u8; 16]).unwrap();
        pile.update(pin_id, None, Some(h1.transmute())).unwrap();
        pile.flush().unwrap();
        ...
        match pile.update(pin_id, Some(h2.transmute()), Some(h1.transmute())) {
            Ok(PushResult::Conflict(existing)) => {
                assert_eq!(existing, Some(h1.transmute()))
            }
            other => panic!("unexpected result: {other:?}"),
        }
        assert_eq!(pile.head(pin_id).unwrap(), Some(h1.transmute()));
        pile.close().unwrap();
    }
    ...
    #[test]
    fn pin_update_conflict_returns_existing_head() {
        let dir = tempfile::tempdir().unwrap();
        let path = fresh_empty_pile_path(&dir, "pile.pile");
        ...
        let pin_id = Id::new([3u8; 16]).unwrap();
        pile.update(pin_id, None, Some(h1.transmute())).unwrap();

        match pile.update(pin_id, Some(h2.transmute()), Some(h2.transmute())) {
            Ok(PushResult::Conflict(existing)) => {
                assert_eq!(existing, Some(h1.transmute()))
            }
    ...
    #[test]
    fn pin_update_noop_does_not_grow_file() {
        let dir = tempfile::tempdir().unwrap();
        let path = fresh_empty_pile_path(&dir, "pile.pile");
        ...
        let pin_id = Id::new([4u8; 16]).unwrap();
        pile.update(pin_id, None, Some(h.transmute())).unwrap();
        pile.flush().unwrap();
        let len_after_first = std::fs::metadata(&path).unwrap().len();

        match pile.update(pin_id, Some(h.transmute()), Some(h.transmute())) {
            Ok(PushResult::Success()) => {}
        ...
        assert_eq!(
            ...
            "no-op pin update must not append a new record"
        );
        assert_eq!(pile.head(pin_id).unwrap(), Some(h.transmute()));
        pile.close().unwrap();
    }
    ...
    /// pin-update no-op rule): the LWW cell carries no new information.
    #[test]
    ...
    /// Mixed pile: a legacy V1 blob and every current V3 record family
    /// interleaved — the scan walks them cleanly and each independent index
    /// resolves correctly.
    #[test]
    fn mixed_v1_v3_record_families_interleave() {
        let dir = tempfile::tempdir().unwrap();
        ...
        let pin_id = Id::new([5u8; 16]).unwrap();
        ...
        // Interleave every current V3 state family with blobs and one legacy
        // V1 record.
    ...
    fn pile_records_walks_mixed_pile_and_fails_loud() {
        ...
        pile.update(pin_id, Some(h1.transmute()), None).unwrap();
        pile.close().unwrap();
        ...
        // Exact sequence: V1 blob, V3 blob, legacy pin set, weak-pin,
        // weak-unpin, legacy pin tombstone.
        ...
        match decoded[2].content {
            PileRecordContent::Pin {
                pin_id: decoded_pin_id,
                head,
            } => {
                assert_eq!(decoded_pin_id, pin_id);
                assert_eq!(head, h1.transmute());
            }
            other => panic!("expected pin record, got {other:?}"),
        }
        ...
        match decoded[5].content {
            ...
            other => panic!("expected pin tombstone record, got {other:?}"),
        }
