...
use crate::query::{
    ActionUnitClasses, CandidateSink, Constraint, EstimateSink, ProposalCoverage, RowsView, Term,
...
/// multiplicity. The constraint deliberately stays structurally opaque so
/// formula lowering cannot split the normalized source back into independently
    ...
    U: Universe,
{
    ...
    /// ordering, duplicate, and tag semantics. `first` makes an empty physical
    /// union unrepresentable.
    ///
    /// # Panics
    ///
...
mod tests {
    ...
    use crate::query::{Binding, ConfirmationUnitClass, ProposalUnitClass, Query, Variable};
    ...
    #[test]
    fn union_archive_satisfaction_is_optimistic_partial_and_exact_physical_or() {
        ...
        let exact = RowsView::new(&vars, &rows);
        assert!(constraint.satisfied(&exact.row_view(0)));
        assert!(constraint.satisfied(&exact.row_view(1)));
        assert!(!constraint.satisfied(&exact.row_view(2)));
        ...
        assert!(constraint.satisfied(&partial.row_view(0)));
        assert!(constraint.satisfied(&partial.row_view(1)));
    }
