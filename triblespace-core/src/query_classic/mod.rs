...
pub trait TriblePattern {
    ...
    /// Each position is either a variable to solve for or a constant value
    /// pinned into the constraint.
...
/// One position of a trible pattern: either a variable the engine solves for
/// or a constant inline value fixed when the constraint is constructed.
#[derive(Debug)]
...
impl<T: InlineEncoding> From<Inline<T>> for Term<T> {
    fn from(constant: Inline<T>) -> Self {
        Self::Const(constant)
    }
}

impl<T: InlineEncoding> Term<T> {
    /// Erase the schema type for storage inside a runtime constraint.
    pub fn erase(self) -> RawTerm {
        match self {
            Self::Var(variable) => RawTerm::Var(variable.index),
            Self::Const(constant) => RawTerm::Const(constant.raw),
        }
    }
}

/// Untyped runtime representation of a [`Term`].
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum RawTerm {
    /// A variable slot index.
    Var(VariableId),
    /// A pinned raw inline value.
    Const(RawInline),
}

impl RawTerm {
    /// Whether this term denotes `variable`.
    #[inline]
    pub fn is_var(&self, variable: VariableId) -> bool {
        matches!(self, Self::Var(candidate) if *candidate == variable)
    }

    /// Resolve this term under `binding`.
    #[inline]
    pub fn bound<'b>(&'b self, binding: &'b Binding) -> Option<&'b RawInline> {
        match self {
            Self::Var(variable) => binding.get(*variable),
            Self::Const(constant) => Some(constant),
        }
    }

    /// Add this term's variable to `set`; constants add nothing.
    #[inline]
...
/// Raw relational identities already admitted by this iterator.
...
/// Claims raw projected tuples before user conversion or mapping.
struct ProjectionGate {
    /// Projected variables in declared head order.
    head: Arc<[VariableId]>,
    claims: ProjectionClaims,
    /// Explicit projections expose only their head to the mapper. `Query::new`
    /// retains the complete original binding.
...
impl ProjectionGate {
    ...
    /// Whether the singleton empty-head identity has already been claimed.
    ...
    #[cfg(feature = "parallel")]
    fn share_for_parallel(&mut self) {
        if let ProjectionClaims::Owned(claims) = &mut self.claims {
            self.claims = ProjectionClaims::Shared(Arc::new(Mutex::new(std::mem::take(claims))));
        }
    }
}
...
impl Clone for ProjectionGate {
    fn clone(&self) -> Self {
        let claims = match &self.claims {
            ...
            #[cfg(feature = "parallel")]
            ProjectionClaims::Shared(claims) => ProjectionClaims::Shared(Arc::clone(claims)),
        };
        Self {
...
impl<'a, C: Constraint<'a>, P: Fn(&Binding) -> Option<R>, R> Query<C, P, R> {
    ...
    /// The complete set of variables named by the constraint is the raw SET
    /// projection head.
    ///
    ...
    /// Construct a query with an explicit ordered relational projection head.
    ///
    /// Raw inline tuples are claimed before conversion or mapper execution, so
    /// hidden witnesses cannot duplicate an output or retry a failed mapping.
    #[doc(hidden)]
    ...
    fn new_inner(
        ...
        // A zero-variable constraint bypasses propose/confirm entirely.
        // At the empty binding `satisfied` is exact because every term is
        // already resolved, so settle that singleton relation up front.
        let mode = if variables.is_empty() && !constraint.satisfied(&binding) {
            Search::Done
        } else {
            Search::NextVariable
        };
            ...
            match &self.mode {
                Search::NextVariable => {
                    ...
                    if self.unbound.is_empty() {
                        match self.projection.project(&self.binding, &self.postprocessing) {
...
mod classic_tests {
    ...
    struct FinitePairs {
        left: VariableId,
        right: VariableId,
        rows: Arc<[(RawInline, RawInline)]>,
    }

    impl FinitePairs {
        fn compatible(&self, binding: &Binding, left: &RawInline, right: &RawInline) -> bool {
            binding.get(self.left).is_none_or(|bound| bound == left)
                && binding.get(self.right).is_none_or(|bound| bound == right)
        }
    }

    impl Constraint<'_> for FinitePairs {
        fn variables(&self) -> VariableSet {
            VariableSet::new_singleton(self.left).union(VariableSet::new_singleton(self.right))
        }

        fn estimate(&self, variable: VariableId, binding: &Binding) -> Option<usize> {
            (variable == self.left || variable == self.right).then(|| {
                self.rows
                    .iter()
                    .filter(|(left, right)| self.compatible(binding, left, right))
                    .count()
            })
        }

        fn propose(&self, variable: VariableId, binding: &Binding, proposals: &mut Vec<RawInline>) {
            if variable != self.left && variable != self.right {
                return;
            }
            for (left, right) in self.rows.iter() {
                if self.compatible(binding, left, right) {
                    proposals.push(if variable == self.left { *left } else { *right });
                }
            }
        }
        ...
        fn confirm(&self, variable: VariableId, binding: &Binding, proposals: &mut Vec<RawInline>) {
            ...
            proposals.retain(|candidate| {
                self.rows.iter().any(|(left, right)| {
                    self.compatible(binding, left, right)
                        && candidate == if variable == self.left { left } else { right }
                })
            });
        }
        ...
        fn satisfied(&self, binding: &Binding) -> bool {
            self.rows
                .iter()
                .any(|(left, right)| self.compatible(binding, left, right))
        }
    }

    #[derive(Clone)]
    struct ZeroVariable(bool);

    impl Constraint<'_> for ZeroVariable {
        fn variables(&self) -> VariableSet {
            VariableSet::new_empty()
        }

        fn estimate(&self, _variable: VariableId, _binding: &Binding) -> Option<usize> {
            None
        }
    ...
    fn duplicate_witnesses() -> FinitePairs {
        FinitePairs {
            left: 0,
            right: 1,
            rows: vec![(raw(1), raw(10)), (raw(1), raw(20))]
                .into_boxed_slice()
                .into(),
        }
    }
    ...
    #[test]
    fn explicit_projection_claims_before_mapping() {
        let calls = Arc::new(AtomicUsize::new(0));
        let observed = Arc::clone(&calls);
        let rows = Query::new_projected(duplicate_witnesses(), [0], move |binding| {
            assert!(binding.get(1).is_none(), "hidden witness leaked to mapper");
    ...
    #[test]
    fn full_and_strict_heads_both_have_raw_set_semantics() {
        let duplicated = FinitePairs {
            left: 0,
            right: 1,
            rows: vec![(raw(1), raw(10)), (raw(1), raw(10)), (raw(1), raw(20))]
                .into_boxed_slice()
                .into(),
        };

        let strict =
            Query::new_projected(duplicated.clone(), [0], |binding| binding.get(0).copied())
                .collect::<Vec<_>>();
        let mut full = Query::new(duplicated, |binding| {
            Some((
        ...
        assert_eq!(strict, vec![raw(1)]);
        assert_eq!(full, vec![(raw(1), raw(10)), (raw(1), raw(20))]);
    }

    #[test]
    fn zero_variable_constraints_are_settled_before_projection() {
        assert_eq!(
            Query::new_projected(ZeroVariable(true), [], |_| Some(())).collect::<Vec<_>>(),
            vec![()]
        );
        assert!(Query::new_projected(ZeroVariable(false), [], |_| Some(()))
            .next()
            .is_none());
    }

    #[test]
    fn output_type_does_not_participate_in_query_auto_traits() {
    ...
    #[cfg(feature = "parallel")]
    #[test]
    fn rayon_splits_share_one_projection_claim_domain() {
        use rayon::iter::{IntoParallelIterator, ParallelIterator};

        let rows = (0..64)
            .map(|witness| (raw(1), raw(witness)))
            .collect::<Vec<_>>()
            .into_boxed_slice();
        let query = Query::new_projected(
            FinitePairs {
                left: 0,
                right: 1,
                rows: rows.into(),
