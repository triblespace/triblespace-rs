...
//! Bit-packed, that is **not** a plain sub-slice: the region's first and last
//! words also carry bits belonging to the *neighbouring* regions of the same
//! buffer. So:
//!
...
impl ProposalBuffer {
    ...
    /// # Cost
    ///
    /// A word-per-candidate layout would concatenate liveness with a `memcpy`.
    /// Packed, `other`'s entry `k` has to land at absolute bit `base + k`, so
    /// unless the join is word-aligned every one of `other`'s words is shifted
    /// left by `base % 32` and its overflow carried into the next word: still
    /// O(words) — over 1/32 of the words a word-per-candidate layout would
    /// copy — but a read-modify-write per word instead of a straight copy, and
    /// not vectorisable as a `memcpy`.
...
// mid-word (so their first word is shared with a neighbour),
// `append`/`split_off`/`rewrite_region` at non-word-aligned indices (so words
// have to be shifted or masked), and anything that hands liveness words in or
// out.
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    fn live_words_reconstruct_is_live_bit_by_bit() {
        ...
        for base in [0usize, 1, 5, 31, 32, 33, 63, 64, 69, 70] {
            ...
            for i in 0..r.len() {
                let bit = offset + i;
                let reconstructed = (words[bit / 32] >> (bit % 32)) & 1 != 0;
                assert_eq!(
                    reconstructed,
                    r.is_live(i),
