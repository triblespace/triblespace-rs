...
struct TypedProgramRuntime<State, NoveltyKey> {
    ...
    /// For every `a`, this equals the number of `slots` whose live owner is
    /// `a`; absence therefore certifies that `a` has no live handle.
    ...
    /// Slot probes performed by retirement itself, excluding explicit model
    /// checks. The count-backed implementation deliberately leaves this zero.
    #[cfg(test)]
    ...
    NoveltyKey: Clone + Eq + Hash + Send + 'static,
{
    ...
    /// Checks the release-mode affine ownership invariant in expected `O(R)`,
    /// independent of the arena high-water mark, then drops novelty state.
...
mod tests {
    ...
    fn stale_handles_are_rejected_after_slot_reuse() {
        let mut runtime = TypedProgramRuntime::<NonComparableState, Key>::default();
        let first = ProgramActivation(1);
        let second = ProgramActivation(2);
        let stale = runtime.insert(first, NonComparableState { exact_cursor: 1 });
        runtime.assert_arena_model();
        let _ = runtime.take(first, stale.clone());
        runtime.assert_arena_model();
        runtime.retire_many(&[first]);
        let fresh = runtime.insert(second, NonComparableState { exact_cursor: 2 });
        ...
        assert!(runtime.contains(&fresh));
        assert_eq!(runtime.live_handles.get(&second), Some(&1));
        runtime.assert_arena_model();
        assert_eq!(runtime.take(second, fresh).exact_cursor, 2);
        runtime.assert_arena_model();
        runtime.retire_many(&[second]);
    }
    ...
    #[test]
    fn retirement_rejects_live_owner_without_disturbing_other_activations() {
        let mut runtime = TypedProgramRuntime::<NonComparableState, Key>::default();
        let dead = ProgramActivation(1);
        let first = ProgramActivation(2);
        let second = ProgramActivation(3);
        assert!(runtime.admit(dead, Key(1), None));
        assert!(runtime.admit(first, Key(1), None));
        assert!(runtime.admit(second, Key(1), None));
        let first_a = runtime.insert(first, NonComparableState { exact_cursor: 1 });
        let first_b = runtime.insert(first, NonComparableState { exact_cursor: 2 });
        let second_handle = runtime.insert(second, NonComparableState { exact_cursor: 3 });
        runtime.assert_arena_model();

        assert_eq!(runtime.take(first, first_a).exact_cursor, 1);
        runtime.assert_arena_model();
        let rejected = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            runtime.retire_many(&[dead, first]);
        }));
        assert!(rejected.is_err());
        assert!(runtime.novelty.contains_key(&dead));
        assert!(runtime.novelty.contains_key(&first));
        assert_eq!(runtime.live_handles.get(&first), Some(&1));
        assert_eq!(runtime.live_handles.get(&second), Some(&1));
        runtime.assert_arena_model();

        assert_eq!(runtime.take(first, first_b).exact_cursor, 2);
        runtime.retire_many(&[dead, first]);
        assert!(!runtime.novelty.contains_key(&dead));
        assert!(!runtime.novelty.contains_key(&first));
        assert!(runtime.novelty.contains_key(&second));
        assert_eq!(runtime.live_handles.get(&second), Some(&1));
        assert_eq!(runtime.take(second, second_handle).exact_cursor, 3);
        runtime.retire_many(&[second]);
        runtime.assert_arena_model();
    }
    ...
    #[test]
    fn zero_root_and_duplicate_retirement_are_idempotent() {
        let mut runtime = TypedProgramRuntime::<NonComparableState, Key>::default();
        let zero_root = ProgramActivation(1);
        let never_seen = ProgramActivation(2);
        assert!(runtime.admit(zero_root, Key(4), None));
        runtime.retire_many(&[zero_root, zero_root, never_seen]);
        assert!(!runtime.novelty.contains_key(&zero_root));
        assert_eq!(runtime.retirement_owner_checks, 3);
    ...
    #[test]
    fn singleton_retirement_never_scans_a_drained_high_water_arena() {
        ...
        for index in 0..RETIREMENTS {
            ...
            assert!(!runtime.novelty.contains_key(&activation));
        }
