...
impl DeltaScheduler {
    ...
    fn step_program<'a>(
        ...
        let validated_examined = validated_program_examined(&scratch.receipt.pages, None);
        let final_source_telemetry_cohort = scratch.receipt.final_source_telemetry_cohort;
        if scratch.receipt.normalized_steps > 0 {
            stats.delta_program_receipt_local_fused_steps += scratch.receipt.normalized_steps;
            stats.delta_program_receipt_local_refiles_avoided += scratch.receipt.normalized_steps;
            stats.max_delta_program_receipt_local_chain = stats
                .max_delta_program_receipt_local_chain
                .max(scratch.receipt.normalized_steps + 1);
        }
        drop(candidate_sets);
        ...
        if scratch.receipt.source_pages > 0 {
            stats.delta_source_cohorts += scratch.receipt.source_cohorts;
            stats.max_delta_source_cohort = stats
                .max_delta_source_cohort
                .max(scratch.receipt.max_source_cohort);
        }
        ...
        if scratch.receipt.transition_pages > 0 {
            stats.delta_transition_cohorts += scratch.receipt.transition_cohorts;
            stats.max_delta_transition_cohort = stats
                .max_delta_transition_cohort
                .max(scratch.receipt.max_transition_cohort);
        }
