...
| Query engine (`src/query/*.rs`) | Constraint solver never aliases conflicting bindings; residual states cover all join permutations referenced by `pattern!`; the fixed specificity key and source frontier always select a sound action. | Expand `proofs/query_harness.rs` with minimal counterexamples, and fuzz constraint graphs via `cargo fuzz`. |
