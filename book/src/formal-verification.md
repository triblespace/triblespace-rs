...
| Inline schemas (`src/value/*.rs`) | Schema encoders respect declared byte widths; `TryFromInline` conversions and `InlineEncoding::validate` reject truncated buffers; zero-copy views stay aligned. | Reuse `value_harness.rs`, adding per-schema helpers plus a Miri regression suite that loads slices at every alignment. |
