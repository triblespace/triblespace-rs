...
| Repository & commits (`src/repo/*.rs`, `proofs/commit_harness.rs`) | Generic pin identity compares the full author key and descriptor handle; verified envelopes form a grow-only set and unknown kinds are preserved; branch descriptors decode canonically; `BranchRank` only suppresses impossible ancestry walks and never decides domination; resolution is insertion-order invariant and retains every true maximal tip; missing data is classified as `TipPending` or `Partial`, never guessed complete; a synthetic flat merge is deterministic derived state and is never silently promoted to an assertion. | Add bounded generic assertion and commit-DAG generators, permute insertion order and duplicates, force identity-digest collisions, vary honest and dishonest labels, and model missing descriptors, tips, and ancestry. |
...
1. Catalogue crate-level invariants and map them to concrete Kani harnesses.
   ...
   - `TribleSet` operations preserving canonical ordering and deduplication.
    - Join heuristics in `atreides` ensuring variable bindings never alias
      conflicting values.
   - Repository assertion resolution maintaining set-union semantics and exact
     identity under arbitrary replay order.
...
1. Introduce a `cargo fuzz` workspace targeting:
   ...
   - Join-order heuristics to explore combinations of constraint graphs and filter
     predicates.
   - Repository workflows by fuzzing sequences of commits, assertion appends,
     partial ancestry arrival, resolution, and merges.
...
1. Model assertion replication scenarios with deterministic event queues to
   explore duplicate/reordered delivery, partial commit-DAG arrival, and
   concurrent writers. The current network has no signed-assertion replication
   protocol, so these models specify the protocol still to be implemented
   rather than claiming coverage of legacy HEAD gossip.
