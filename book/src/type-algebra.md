...
In the codebase the macro emits a `static LazyLock<Attribute<S>>` for each declared attribute, so the generated binding already carries the `ValueSchema` that governs the value column.
