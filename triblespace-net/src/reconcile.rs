//! Want-reconcile: service this peer's durable asserted **wants** by fetching
//! absent blobs from the swarm.
//!
//! A want is a signed, grow-only assertion: "this author would like this
//! blob; fetch it if absent." A long-running sync daemon (`trible pile net
...
/// wants themselves live durably in the asserted-pin store.
struct WantState {
...
/// bookkeeping — the wants themselves are durable assertions, so
...
impl Reconciler {
        ...
        S: BlobStore + BlobStorePut + PinStore + PinAssertionStore + StorageFlush + Send + 'static,
    {
        ...
        // out-of-band (for example, another fetch landed the blob).
        ...
        for hash in missing {
            ...
            match peer.fetch_blob_with_deadline(hash, self.fetch_budget).await {
                Some(bytes) => {
                    ...
                    // them). The assertion that expressed the want is already
                    // on record; no assertion or retention state changes here.
                ...
                None => {
                    ...
                    // the want stays on record, retried
