...
- **One all-variable `QueryProgram` pattern can run as a forced resident
  ...
  columns directly into its poison-filled packed result, and performs one
  final read even for zero seeds or an empty archive. An explicit O(domain)
  paired-select E scan, changed-E/A LF navigation, stable exact-capacity scans,
  checked geometry, and sticky device invariants preserve the forced CPU
  interpreter's seed-major order and multiplicity.
