...
- **Known archive-entry materialization reuses its established PATCH hash.**
  Singleton and owner-mismatch archive insertion write the `ArchiveEntry` hash
  they already carry into the allocated heap Leaf. Generic LocalLeaf
  reification skips redundant process-key initialization while preserving the
  ordinary allocation-before-hashing path.
