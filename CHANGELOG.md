...
- **Experimental PATCH union can leave internal hashes lazy.** A zero Branch
  ...
  consume them, and cached input aggregates repair the result algebraically.
  Disjoint archive-backed aggregation and duplicate union below wholly dirty
  roots can therefore move or reuse LocalLeaves without hashing them. The large
  parallel scatter carries demanded receipts in a transient scalar sidecar and
  rebuilds only structural aggregates, rather than recursively hashing dirty
  children during bulk finalization. On-demand root hashes and deep debug
  audits retain the same key-set semantics.
