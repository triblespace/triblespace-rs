...
fn estimate_is_universe_code_range_upper_bound() {
    ...
    use triblespace::core::query::{Candidates, RowsView};
    let mut est = Vec::new();
    assert!(
        constraint.estimate(v.index, RowsView::EMPTY, &mut est),
    ...
    constraint.propose(v.index, RowsView::EMPTY, &mut proposals);
    assert_eq!(
        proposals.len(),
        3,
