...
fn propose_and_confirm() {
    ...
    constraint.propose(a_var.index, view, &mut proposals);
    ...
    constraint.confirm(a_var.index, view, &mut proposals);
...
fn propose_and_confirm_bound_attribute() {
    ...
    let mut proposals: Candidates = Vec::new();
    constraint.propose(e_var.index, view, &mut proposals);
    ...
    constraint.confirm(e_var.index, view, &mut proposals);
    assert_eq!(proposals.len(), 2);
}
